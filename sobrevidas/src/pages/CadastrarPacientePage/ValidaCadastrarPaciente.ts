...
export function ValidaCadastrarPaciente(newPaciente: Models.Paciente) {
  ...
  msgErro += ValidaTelefone(
    newPaciente.telefoneResponsavel,
    'Telefone Responsável',
  );

  if (msgErro.length > 0) {
    Alert.alert('Atenção', 'Verifique os seguintes campos:\n' + msgErro);
    return false;
  } else {
    return true;
  }
...
function ValidaCPF(cpf: String): String {
  if (cpf && cpf.length === 14) {
    return '';
  } else if (!cpf || cpf === null || cpf === '' || cpf.length === 0) {
    return 'CPF não preenchido\n';
  } else if (cpf.length > 0 && cpf.length < 14) {
    return 'CPF preenchido incorretamente\n';
  } else {
    return 'Verifique CPF\n';
  }
}

function ValidaDataNascimento(dtNascimento: String): String {
  const ValidaLength = dtNascimento && dtNascimento.length === 10;
  const ValidaBarras = ValidaLength
    ? dtNascimento[2] === '/' || dtNascimento[5] === '/'
    : false;
  const [dd, mm, aa] =
    ValidaLength && ValidaBarras ? dtNascimento.split('/') : [];
...
function ValidaEmail(email: String): String {
  return (!email || email.length === 0 || email.search('@')) === -1
    ? 'Email Inválido\n'
    : '';
}

function ValidaEndereco(enderecoCompleto: String): String {
  return !enderecoCompleto || enderecoCompleto.length < 4
    ? 'Endereço Inválido\n'
    : '';
}
